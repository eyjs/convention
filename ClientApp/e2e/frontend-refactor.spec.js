...
test.describe('1. 인증 흐름', () => {
  ...
  test('미인증 상태에서 보호 페이지 접근 시 /login 리다이렉트', async ({
    page,
  }) => {
    await page.goto('/')
    await expect(page).toHaveURL(/\/login/)
...
test.describe('3. Convention 라우팅', () => {
  ...
  test('존재하지 않는 Convention ID → MainHome 리다이렉트', async ({
    page,
  }) => {
  ...
  test('Bottom nav 일정 탭 클릭 → /conventions/:id/schedule', async ({
    page,
  }) => {
    test.skip(!conventionId, 'No convention available')
    ...
    await page.waitForURL(`**/conventions/${conventionId}/schedule`, {
      timeout: 5000,
    })
    expect(page.url()).toContain(`/conventions/${conventionId}/schedule`)
  })

  test('Bottom nav 게시판 탭 클릭 → /conventions/:id/notices', async ({
    page,
  }) => {
    test.skip(!conventionId, 'No convention available')
    ...
    await page.waitForURL(`**/conventions/${conventionId}/notices`, {
      timeout: 5000,
    })
    expect(page.url()).toContain(`/conventions/${conventionId}/notices`)
  })

  test('Bottom nav 더보기 탭 클릭 → /conventions/:id/features', async ({
    page,
  }) => {
    test.skip(!conventionId, 'No convention available')
    ...
    await page.waitForURL(`**/conventions/${conventionId}/features`, {
      timeout: 5000,
    })
    expect(page.url()).toContain(`/conventions/${conventionId}/features`)
  })
  ...
  test('Bottom nav 홈 탭 클릭 → /conventions/:id', async ({ page }) => {
    ...
    await page.waitForURL(
      (url) => {
        ...
        return (
          pathname === `/conventions/${conventionId}` ||
          pathname === `/conventions/${conventionId}/`
        )
      },
      { timeout: 5000 },
...
test.describe('4. localStorage 정리 확인', () => {
  ...
  test('로그인 후 localStorage에 selectedConventionId 없음', async ({
    page,
  }) => {
    await login(page)
    const cid = await page.evaluate(() =>
      localStorage.getItem('selectedConventionId'),
    )
    expect(cid).toBeNull()
  })
...
test.describe('5. 레거시 리다이렉트', () => {
  ...
  test('/home → /', async ({ page }) => {
    ...
    await page.waitForURL((url) => !url.pathname.includes('/home'), {
      timeout: 5000,
    })
  ...
  test('/my-schedule → /', async ({ page }) => {
    ...
    await page.waitForURL((url) => !url.pathname.includes('/my-schedule'), {
      timeout: 5000,
    })
