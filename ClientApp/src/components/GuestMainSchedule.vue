  ...
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <div v-if="currentSchedule" class="p-4">
      <div class="bg-white rounded-lg shadow-lg p-5 border-l-4 border-green-500">
        ...
        <div class="flex items-center gap-3 mb-2">
          <span class="text-xl font-bold text-primary-600">{{ formatTime(currentSchedule.scheduleDate) }}</span>
          <h3 class="text-lg font-bold">{{ currentSchedule.title }}</h3>
        </div>
        <p v-if="currentSchedule.location" class="text-sm text-gray-600 flex items-center gap-1 mb-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
          </svg>
          {{ currentSchedule.location }}
        </p>
        <p v-if="currentSchedule.content" class="text-gray-700 text-sm">{{ currentSchedule.content }}</p>
      </div>
    </div>
    ...
    <div v-if="nextSchedule" class="px-4 pb-4">
      ...
      <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
        <div class="flex items-center gap-3 mb-2">
          <span class="text-lg font-bold text-primary-600">{{ formatTime(nextSchedule.scheduleDate) }}</span>
          <h3 class="text-base font-semibold">{{ nextSchedule.title }}</h3>
        </div>
        <p v-if="nextSchedule.location" class="text-sm text-gray-600 flex items-center gap-1">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
          </svg>
          {{ nextSchedule.location }}
        </p>
        <p v-if="timeUntilNext" class="text-xs text-blue-600 mt-2">{{ timeUntilNext }} 후 시작</p>
      </div>
    </div>
    ...
    <div class="px-4 pb-4">
      ...
      <div v-if="todaySchedules.length === 0" class="bg-white rounded-lg shadow p-6 text-center text-gray-500">
        오늘 일정이 없습니다
      </div>
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div class="flex items-center gap-3">
            <span class="text-base font-bold text-primary-600 w-16">{{ formatTime(schedule.scheduleDate) }}</span>
            <div class="flex-1">
              <p class="font-semibold">{{ schedule.title }}</p>
              <p v-if="schedule.location" class="text-xs text-gray-500">{{ schedule.location }}</p>
            </div>
            <svg v-if="isPast(schedule.scheduleDate)" class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div v-if="tomorrowSchedules.length > 0" class="px-4 pb-4">
      ...
      <div class="space-y-2">
        ...
        >
          <div class="flex items-center gap-3">
            <span class="text-base font-bold text-gray-600 w-16">{{ formatTime(schedule.scheduleDate) }}</span>
            <div class="flex-1">
              <p class="font-semibold">{{ schedule.title }}</p>
              <p v-if="schedule.location" class="text-xs text-gray-500">{{ schedule.location }}</p>
            </div>
...
const loadSchedules = async () => {
  try {
    const response = await apiClient.get(`/api/guestschedule/guests/${props.guestId}/today-next`)
