<template>
  <div class="max-w-4xl mx-auto p-4">
    ...
    <!-- 날짜별 그룹 -->
    <div v-for="(dateGroup, dateKey) in groupedSchedules" :key="dateKey" class="mb-8">
      <div class="sticky top-0 bg-white z-10 py-3 border-b-2 border-primary-600">
        <h3 class="text-xl font-bold text-primary-600">{{ formatDateHeader(dateKey) }}</h3>
      </div>
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <p v-if="schedule.content" class="text-gray-700 whitespace-pre-wrap leading-relaxed">
                {{ schedule.content }}
              </p>
            </div>
          </div>
    ...
    <div v-if="Object.keys(groupedSchedules).length === 0" class="text-center py-12 text-gray-500">
      등록된 일정이 없습니다
    </div>
  </div>
</template>
...
const groupedSchedules = computed(() => {
  ...
  Object.keys(groups).forEach(key => {
    groups[key].sort((a, b) => new Date(a.scheduleDate) - new Date(b.scheduleDate))
  })
