<template>
  <div class="bg-white rounded-lg shadow-sm p-6">
    <h2 class="text-xl font-semibold mb-4">챗봇 색인 관리</h2>

    <!-- 색인 현황 -->
    <div class="mb-6">
      <h3 class="text-lg font-medium text-gray-800 mb-2">현재 상태</h3>
      <div class="bg-gray-50 rounded-lg p-4 flex items-center justify-between">
        <p class="text-gray-600">색인된 문서 수</p>
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">행사의 모든 정보를 다시 읽어 챗봇의 지식 베이스를 업데이트합니다. 데이터 양에 따라 시간이 걸릴 수 있습니다.</p>
          </div>
          <button
            @click="handleReindex"
            :disabled="reindexing"
            class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-wait transition-colors"
          >
...
async function fetchStatus() {
  ...
  try {
    const response = await apiClient.get(`/admin/conventions/${props.conventionId}/indexing/status`)
...
async function handleReindex() {
  ...
  resultMessage.value = ''
  try {
    const response = await apiClient.post(`/admin/conventions/${props.conventionId}/indexing/reindex`)
    resultMessage.value = response.data.message
    resultType.value = 'success'
    await fetchStatus() // 상태 다시고침
  } catch (error) {
