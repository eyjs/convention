  ...
  <div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <div v-if="status" class="mb-6 p-4 rounded-lg border" :class="statusClass">
      <div class="flex items-center space-x-2 mb-2">
        <svg v-if="status.canConnect" class="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 20 20">
        ...
        <span class="font-semibold">{{ status.canConnect ? 'DB 연결 성공' : 'DB 연결 실패' }}</span>
      </div>

      <div class="text-sm space-y-1">
        <div><strong>데이터베이스:</strong> {{ status.databaseExists ? '존재함' : '없음' }}</div>
        <div><strong>적용된 Migration:</strong> {{ status.appliedMigrations?.length || 0 }}개</div>
        <div><strong>대기 중인 Migration:</strong> {{ status.pendingMigrations?.length || 0 }}개</div>
        <div v-if="status.needsMigration" class="text-orange-600 font-semibold">⚠️ Migration 필요</div>
        <div v-if="status.error" class="text-red-600 mt-2">{{ status.error }}</div>
      </div>
    </div>
    ...
    <div v-if="analyses.length > 0" class="mb-6 space-y-4">
      <div class="flex items-center justify-between">
        ...
        >
          ...
          <svg class="w-4 h-4" :class="{ 'rotate-180': showAnalysisDetails }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Transition name="slide-fade">
        <div v-if="showAnalysisDetails" class="space-y-4">
          <div v-for="analysis in analyses" :key="analysis.migrationId" class="border rounded-lg p-4" :class="getRiskBorderClass(analysis.riskLevel)">
            <!-- Migration 헤더 -->
            <div class="flex items-start justify-between mb-3">
              <div>
                <h5 class="font-bold text-gray-900">{{ analysis.migrationName }}</h5>
                <p class="text-xs text-gray-500 mt-1">{{ analysis.migrationId }}</p>
              </div>
              <div class="flex items-center space-x-2">
                <span class="px-3 py-1 rounded-full text-sm font-bold" :class="getRiskBadgeClass(analysis.riskLevel)">
                  {{ analysis.overallRisk }}
                </span>
                <span v-if="analysis.requiresBackup" class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">백업 필요</span>
                <span v-if="analysis.requiresReview" class="px-2 py-1 bg-red-100 text-red-800 text-xs rounded">수동 검토</span>
              </div>
            </div>

            <!-- 위험 항목 목록 -->
            <div v-if="analysis.risks && analysis.risks.length > 0" class="mb-3">
              <h6 class="text-sm font-semibold text-gray-700 mb-2">🔍 감지된 변경 사항:</h6>
              <div class="space-y-2">
                <div v-for="(risk, idx) in analysis.risks" :key="idx" class="bg-gray-50 rounded p-3 border-l-4" :class="getRiskBorderColorClass(risk.levelValue)">
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center space-x-2 mb-1">
                        <span class="text-xs font-bold px-2 py-0.5 rounded" :class="getRiskLevelBadge(risk.levelValue)">{{ risk.level }}</span>
                        <span class="text-xs font-semibold text-gray-700">{{ risk.type }}</span>
                        <span v-if="risk.table" class="text-xs text-gray-500">테이블: {{ risk.table }}</span>
                        <span v-if="risk.column" class="text-xs text-gray-500">컬럼: {{ risk.column }}</span>
                      </div>
                      <p class="text-sm text-gray-900 mb-1">{{ risk.description }}</p>
                      <p class="text-xs text-gray-600 mb-1"><strong>영향:</strong> {{ risk.impact }}</p>
                      <p v-if="risk.mitigation" class="text-xs text-blue-600"><strong>조치:</strong> {{ risk.mitigation }}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <!-- 영향받는 테이블 -->
            <div v-if="analysis.affectedTables && analysis.affectedTables.length > 0" class="mb-3">
              <h6 class="text-sm font-semibold text-gray-700 mb-2">📊 영향받는 테이블:</h6>
              <div class="bg-blue-50 border border-blue-200 rounded p-3">
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr v-for="table in analysis.affectedTables" :key="table.tableName" class="border-b border-blue-100 last:border-0">
                      <td class="py-1 font-semibold">{{ table.tableName }}</td>
                      <td class="py-1">{{ table.currentRowCount >= 0 ? table.currentRowCount.toLocaleString() + ' 행' : '알 수 없음' }}</td>
                      <td class="py-1 text-xs text-gray-600">{{ table.operation }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div v-if="analysis.recommendation" class="bg-gray-100 rounded p-3">
              <h6 class="text-sm font-semibold text-gray-700 mb-2">💡 권장 사항:</h6>
              <p class="text-sm text-gray-800 whitespace-pre-line">{{ analysis.recommendation }}</p>
            </div>
          </div>
        </div>
      </Transition>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
      ...
      >
        <svg v-if="hasCriticalRisk" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
    ...
    <!-- Critical Risk 경고 -->
    <div v-if="hasCriticalRisk && !userApprovedCritical" class="mb-6 bg-red-50 border-2 border-red-500 rounded-lg p-4">
      <div class="flex items-start space-x-3">
        <svg class="w-6 h-6 text-red-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
        </svg>
        <div class="flex-1">
          <h5 class="font-bold text-red-900 mb-2">⚠️⚠️ 매우 위험한 Migration 감지!</h5>
          <p class="text-sm text-red-800 mb-3">
            데이터 손실 위험이 있는 작업(DROP COLUMN, DROP TABLE 등)이 포함되어 있습니다.
          </p>
    ...
    <div v-if="logs.length > 0" class="mt-6">
      <div class="flex items-center justify-between mb-2">
        ...
        <button @click="logs = []" class="text-xs text-gray-500 hover:text-gray-700">지우기</button>
      </div>
      <div class="bg-gray-900 text-green-400 rounded-lg p-4 font-mono text-xs max-h-64 overflow-y-auto">
...
function getRiskBorderClass(riskLevel) {
  switch(riskLevel) {
    case 4: return 'border-red-500 bg-red-50'
    case 3: return 'border-orange-500 bg-orange-50'
...
async function checkStatus() {
  ...
  try {
    ...
    addLog(`[SUCCESS] 상태 확인 완료: ${response.data.canConnect ? '연결 성공' : '연결 실패'}`, 'SUCCESS')

    if (response.data.needsMigration) {
      addLog(`[WARN] ${response.data.pendingMigrations.length}개의 Migration이 대기 중입니다.`, 'WARN')
      // 자동으로 분석 실행
    ...
    addLog(`[ERROR] 상태 확인 실패: ${error.response?.data?.error || error.message}`, 'ERROR')
...
async function analyzeMigrations() {
  ...
  try {
    ...
    if (analyses.value.length > 0) {
      addLog(`[SUCCESS] ${analyses.value.length}개의 Migration 분석 완료`, 'SUCCESS')

      const criticalCount = analyses.value.filter(a => a.riskLevel >= 4).length
      const highCount = analyses.value.filter(a => a.riskLevel === 3).length

      if (criticalCount > 0) {
        addLog(`[WARN] ⚠️⚠️ ${criticalCount}개의 매우 위험한 Migration 감지!`, 'WARN')
      }
      if (highCount > 0) {
    ...
    addLog(`[ERROR] 분석 실패: ${error.response?.data?.detail || error.message}`, 'ERROR')
  } finally {
    loading.value = false
  }
}
...
async function testConnection() {
  ...
  try {
    ...
    addLog(`[ERROR] 연결 테스트 실패: ${error.response?.data?.detail || error.message}`, 'ERROR')
  } finally {
    loading.value = false
  }
}
...
async function dryRunMigration() {
  ...
  try {
    ...
    addLog(`[INFO] 적용 예정 Migration: ${response.data.pendingMigrations?.join(', ')}`)
  } catch (error) {
    addLog(`[ERROR] Dry Run 실패: ${error.response?.data?.detail || error.message}`, 'ERROR')
  } finally {
    loading.value = false
  }
}
...
async function confirmMigration() {
  ...
  try {
    const response = await apiClient.post('/admin/database/migrate?dryRun=false')
    addLog(`[SUCCESS] ${response.data.message}`, 'SUCCESS')
    addLog(`[INFO] 실행된 Migration: ${response.data.executedMigrations?.join(', ')}`)
    ...
    addLog(`[ERROR] Migration 실패: ${error.response?.data?.detail || error.message}`, 'ERROR')
