  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-600 mt-1">페이지별로 버튼, 배너, 팝업 등 동적 기능을 추가하고 관리합니다</p>
      </div>
      <button
    ...
    <div v-else-if="features.length > 0" class="space-y-6">
      <div v-for="(group, page) in groupedFeatures" :key="page" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <!-- 페이지 헤더 -->
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center space-x-3 mb-2">
                  <h4 class="text-base font-bold text-gray-900">{{ feature.actionName }}</h4>

                  <!-- Category 배지 -->
                  <span :class="getCategoryBadgeClass(feature.actionCategory)" class="px-2 py-1 text-xs font-medium rounded">
                  ...
                  <span
                    ...
                    :class="feature.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'"
                  >
                ...
                <div class="space-y-2 text-sm">
                  ...
                  <div class="flex items-center text-gray-600">
                    ...
                    <span class="font-medium">{{ getLocationDisplayName(feature.targetLocation) }}</span>
                  </div>

                  <!-- 설정 미리보기 -->
                  <div v-if="feature.config" class="flex items-start text-gray-600">
                    ...
                    <div class="flex-1">
                      ...
                      >
                        {{ expandedConfigs.includes(feature.id) ? '설정 숨기기' : '설정 보기' }}
                      </button>
                      <pre
              ...
              <div class="flex items-center space-x-2 ml-4">
                ...
                >
                  <svg v-if="feature.isActive" class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-else class="text-center py-12 bg-white rounded-lg border-2 border-dashed border-gray-300">
      ...
      <h3 class="text-lg font-medium text-gray-900 mb-2">등록된 기능이 없습니다</h3>
      <p class="text-gray-600 mb-4">버튼, 배너, 팝업 등 동적 기능을 추가해보세요</p>
      <button
        @click="openCreateModal"
        ...
        <form @submit.prevent="saveFeature" class="p-6 space-y-5">
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">
              기능 유형 * <span class="text-xs font-normal text-gray-500">(The What)</span>
            </label>
            <select
          ...
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">
              표시 위치 * <span class="text-xs font-normal text-gray-500">(The Where)</span>
            </label>
            <select
            ...
            <p v-if="selectedLocation" class="text-xs text-gray-600 mt-1">
              <strong>{{ selectedLocation.page }}</strong>: {{ selectedLocation.description }}
            </p>
            <p v-else-if="form.actionCategory && filteredLocationOptions.length === 0" class="text-xs text-red-600 mt-1">
          ...
          <div>
            ...
            ></textarea>
            <div class="mt-2 p-3 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
              <strong>설정 예시:</strong>
...
const filteredLocationOptions = computed(() => {
  ...
  const allowedLocations = getAllowedLocationsForCategory(form.value.actionCategory)
...
async function loadFeatures() {
  ...
  try {
    ...
    features.value = (response.data || []).filter(f => f.actionCategory && f.targetLocation)
  } catch (error) {
...
function openEditModal(feature) {
  ...
  form.value = {
    ...
    config: typeof feature.config === 'string' ? feature.config : JSON.stringify(feature.config, null, 2),
...
async function saveFeature() {
  ...
  try {
    ...
    const payload = {
      ...
      actionType: editingFeature.value?.actionType || `${form.value.actionCategory}_${Date.now()}`,
      mapsTo: 'DYNAMIC_FEATURE' // 동적 기능 표시
    }

    if (editingFeature.value) {
      await apiClient.put(`/conventions/${props.conventionId}/actions/${editingFeature.value.id}`, payload)
    } else {
    ...
    errorMessage.value = error.message || error.response?.data?.message || '저장에 실패했습니다.'
  } finally {
...
async function toggleFeature(feature) {
  try {
    await apiClient.put(`/conventions/${props.conventionId}/actions/${feature.id}`, {
      ...feature,
      isActive: !feature.isActive
    })
...
async function deleteFeature(feature) {
  ...
  try {
    await apiClient.delete(`/conventions/${props.conventionId}/actions/${feature.id}`)
    await loadFeatures()
  } catch (error) {
