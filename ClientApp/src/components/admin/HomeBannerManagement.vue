<template>
  <div>
    ...
    <p class="text-sm text-gray-500 mb-4">
      메인 홈 상단에 표시되는 캐러셀 배너입니다. 드래그하여 순서를 변경할 수
      있습니다.
    </p>
          ...
          <div class="flex flex-col sm:flex-row sm:items-stretch">
            ...
            <div class="relative flex-shrink-0">
              <img
                ...
                class="w-full h-32 sm:w-32 sm:h-20 object-cover"
                alt=""
              />
              <div
                class="drag-handle absolute top-2 left-2 p-1 bg-black/40 rounded cursor-grab active:cursor-grabbing text-white/80"
              >
            ...
            <!-- 정보 + 액션 -->
            <div
              class="flex-1 min-w-0 p-3 flex items-center justify-between gap-2"
            >
              <div class="min-w-0">
                ...
                <p
                  v-if="element.linkUrl"
                  class="text-xs text-gray-400 truncate"
                >
                  {{ element.linkUrl }}
                </p>
                <p
                  v-else-if="!element.detailImagesJson"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              ...
              <span class="text-xs text-gray-400 font-normal ml-1"
                >클릭 시 이미지가 이어붙여진 상세 페이지로 이동</span
              >
            </label>
            <div v-if="form.detailImages.length > 0" class="space-y-2 mb-2">
              ...
              >
                <template #item="{ element, index }">
                  ...
                  >
                    <div
                      class="detail-drag cursor-grab text-gray-300 hover:text-gray-500"
                    >
                      <svg
                        class="w-4 h-4"
                        fill="currentColor"
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-400">
                {{ form.isActive ? '메인 홈에 표시됨' : '표시 안 됨' }}
              </p>
            </div>
            <button
              type="button"
...
function openEditModal(banner) {
  ...
  if (banner.detailImagesJson) {
    try {
      detailImages = JSON.parse(banner.detailImagesJson)
    } catch {
      detailImages = []
    }
  }
...
async function onFileSelect(e) {
  ...
  try {
    ...
    const res = await apiClient.post(
      '/file/upload/image?dateFolder=banners',
      fd,
...
async function saveBanner() {
  ...
  try {
    const payload = {
      ...
      detailImagesJson:
        form.value.detailImages.length > 0
          ? JSON.stringify(form.value.detailImages)
          : null,
      isActive: form.value.isActive,
    }
    if (editingBanner.value) {
      await apiClient.put(
        `/admin/home-banners/${editingBanner.value.id}`,
        payload,
      )
    } else {
