      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">
            {{ types.find((t) => t.value === form.type)?.desc }}
          </p>
        </div>
        ...
        <div v-if="form.type === 'NOTICE'">
          ...
          >
            ...
            <option v-for="n in notices" :key="n.id" :value="n.id">
              {{ n.title }}
            </option>
          </select>
        </div>
        ...
        <div v-if="form.type === 'SEAT'">
          ...
          >
            <option :value="null">선택...</option>
            <option v-for="sl in seatingLayouts" :key="sl.id" :value="sl.id">
              {{ sl.name }}
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          <div class="flex gap-3">
            <label class="flex items-center gap-1.5"
              ><input v-model="form.targetScope" type="radio" value="ALL" />
              전체</label
            >
            <label class="flex items-center gap-1.5"
              ><input v-model="form.targetScope" type="radio" value="GROUP" />
              그룹별</label
            >
            <label class="flex items-center gap-1.5"
              ><input
          ...
          <select
            v-if="form.targetScope === 'GROUP'"
            v-model="form.targetGroupName"
          ...
          <div v-if="form.targetScope === 'INDIVIDUAL'" class="mt-2 space-y-2">
            ...
            />
            <div
              class="max-h-40 overflow-y-auto border rounded-lg p-2 space-y-1"
            >
              <label
              ...
              >
                ...
                <span class="text-gray-400 text-xs">{{
                  g.corpPart || g.groupName || ''
                }}</span>
              </label>
            </div>
            <p class="text-xs text-gray-500">
              {{ form.targetUserIds?.length || 0 }}명 선택
            </p>
          </div>
        </div>
        ...
        <div v-if="statsData" class="space-y-4">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <span
                ...
                :class="
                  statsData.read === statsData.total
                    ? 'text-green-600'
                    : 'text-blue-600'
                "
              >
                {{
                  statsData.total > 0
                    ? Math.round((statsData.read / statsData.total) * 100)
                    : 0
                }}%
              </span>
            </div>
            <div class="h-3 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                :style="{
                  width:
                    statsData.total > 0
                      ? `${(statsData.read / statsData.total) * 100}%`
                      : '0%',
                }"
              ></div>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="border rounded-lg p-3">
              <h4
                class="text-sm font-semibold text-green-700 mb-2 flex items-center gap-1"
              >
              ...
              <div class="max-h-60 overflow-y-auto space-y-1">
                <div
                  v-for="u in statsData.readUsers"
                  :key="u.name"
                  class="flex items-center justify-between text-sm py-1 px-2 rounded hover:bg-green-50"
                >
                  <span>{{ u.name }}</span>
                  <span class="text-xs text-gray-400">{{
                    u.readAt
                      ? new Date(u.readAt).toLocaleString('ko-KR', {
                          month: 'numeric',
                          day: 'numeric',
                          hour: '2-digit',
                          minute: '2-digit',
            ...
            <div class="border rounded-lg p-3">
              <h4
                class="text-sm font-semibold text-red-700 mb-2 flex items-center gap-1"
              >
              ...
              <div class="max-h-60 overflow-y-auto space-y-1">
                <div
                  v-for="u in statsData.unreadUsers"
                  :key="u.name"
                  class="text-sm py-1 px-2 rounded hover:bg-red-50"
                >
                  {{ u.name }}
                </div>
              </div>
...
const types = [
  {
    ...
    label: '텍스트',
    icon: '💬',
    desc: '자유 텍스트 전달 (현장 긴급 안내 등)',
  },
  {
    value: 'NOTICE',
    label: '공지',
    icon: '📢',
    desc: '게시판 공지를 연결하여 알림',
  },
  {
    value: 'SURVEY',
    label: '설문',
    icon: '📋',
    desc: '설문조사 독촉/안내 (딥링크 포함)',
  },
  {
    value: 'SCHEDULE',
    label: '일정',
    icon: '📅',
    desc: '일정 변경/추가 안내 (일정 페이지로 이동)',
  },
  {
    value: 'SEAT',
    label: '좌석',
    icon: '💺',
    desc: '좌석 배정 완료 알림 (내 자리 보기로 이동)',
  },
  {
    value: 'LINK',
    label: '링크',
    icon: '🔗',
    desc: '외부 URL 또는 커스텀 링크 전달',
  },
]
...
const guestCount = computed(() => {
  if (form.targetScope === 'GROUP' && form.targetGroupName)
    return guests.value.filter((g) => g.groupName === form.targetGroupName)
      .length
  if (form.targetScope === 'INDIVIDUAL') return form.targetUserIds?.length || 0
...
async function loadData() {
  try {
    ...
    groups.value = [
      ...new Set(guests.value.map((g) => g.groupName).filter(Boolean)),
    ].sort()

    // 공지/설문 목록
    const [noticeRes, surveyRes] = await Promise.all([
      apiClient
        .get(`/admin/notices?conventionId=${props.conventionId}`)
        .catch(() => ({ data: { items: [] } })),
      apiClient
        .get(`/surveys/convention/${props.conventionId}`)
        .catch(() => ({ data: [] })),
    ])
...
async function send() {
  ...
  try {
    const res = await apiClient.post(
      `/admin/conventions/${props.conventionId}/notifications`,
      form,
    )
