<template>
  <div
    class="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-gray-100 font-sans-kr"
  >
    <!-- [좌측] 설정 패널 -->
    <aside
      class="no-print w-full md:w-96 bg-white border-r border-gray-200 flex flex-col h-full z-50 shadow-xl shrink-0"
    >
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            >
              ...
              <div class="ml-3">
                <span class="block text-sm font-medium text-gray-900"
                  >테이블 안내문 (Table Sign)</span
                >
                <span class="block text-xs text-gray-500"
                  >A4 용지 1장에 전체 좌석 표시</span
                >
              </div>
            </label>
            <label
        ...
        <div
          ...
          :class="{
            'opacity-30 pointer-events-none': config.mode === 'individual',
          }"
        >
          <h2
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="mt-1 text-[10px] text-gray-400">
              권장 규격: <strong>가로 500px 이상 (배경없는 PNG)</strong>
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <button
                ...
                @click="config.font = 'font-sans-kr'"
              >
                본고딕
              </button>
              <button
                ...
                @click="config.font = 'font-serif-kr'"
              >
                나눔명조
              </button>
            </div>
        ...
        <div class="space-y-3">
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-lg p-3">
            ...
            <div class="flex flex-col gap-1 mt-2">
              ...
              >
              <button
                class="text-[10px] text-red-500 hover:underline self-end"
                @click="clearBg('full')"
              >
                삭제
              </button>
    ...
    <!-- [우측] 미리보기 영역 -->
    <main
      class="flex-1 overflow-auto bg-gray-200/80 p-8 md:p-12 relative flex justify-center"
    >
      <div id="previewContainer" class="flex flex-col items-center gap-8 w-fit">
        ...
        <div
          ...
          :class="[
            config.font,
            config.mode === 'individual' ? 'mode-individual' : 'mode-table',
          ]"
          :style="pageStyle"
        >
          <!-- [MODE 1] 개별 명찰 모드 (4분할) -->
            ...
            >
              <template v-if="guest">
                ...
                <div class="tent-top grayscale opacity-60">
                  <div
                    class="text-[10px] font-bold text-gray-500 mb-1 tracking-widest uppercase"
                  >
                    {{ config.eventName }}
                  </div>
                  <img
                  ...
                  />
                  <div
                    class="text-3xl font-bold mb-1 whitespace-nowrap"
                ...
                <div class="tent-bottom">
                  <div
                    class="text-[10px] font-bold text-gray-400 mb-2 tracking-[0.25em] uppercase"
                  >
                    {{ config.eventName }}
                  </div>
                  <img
                  ...
                  />
                  <div
                    class="text-5xl font-black mb-2 leading-tight whitespace-nowrap tracking-tight"
                    :class="currentTheme.text"
                  >
                    {{ guest.name }}
                  </div>
          ...
          <template v-else>
            <div class="table-header" :class="currentTheme.border">
              <div class="flex flex-col">
                <span
                  ...
                  >{{ config.eventName }}</span
                >
                <span class="text-5xl font-black" :class="currentTheme.text">{{
                  page.tableName
                }}</span>
              </div>
              <img
            ...
            <div class="mt-2 text-right w-full text-[10px] text-gray-400">
              총 {{ page.realCount }}명
            </div>
          </template>
        </div>
      ...
      <div
        class="no-print fixed bottom-6 left-1/2 md:left-[calc(50%+192px)] transform -translate-x-1/2 text-center pointer-events-none"
      >
        <span
          class="bg-black/70 text-white text-xs px-4 py-2 rounded-full backdrop-blur-sm shadow-lg"
        >
...
const statusMessage = computed(() => {
  ...
  const tableCount = new Set(guestsData.value.map((g) => g.table || 'No Table'))
    .size
...
const clearBg = (type) => {
  bgImages[type] = null
  if (type === 'full') {
    const fullBgInput = document.querySelector(
      'input[type="file"][accept="image/*"]:first-of-type',
    )
    if (fullBgInput) fullBgInput.value = ''
  } else {
    const cardBgInput = document.querySelector(
