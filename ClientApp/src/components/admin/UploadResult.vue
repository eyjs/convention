<template>
  <div v-if="result" class="mt-4">
    <div
      :class="['p-4 rounded-md', result.success ? 'bg-green-50' : 'bg-red-50']"
    >
      <p
        class="font-medium"
      ...
      <!-- 참석자 업로드 결과 -->
      <div
        v-if="type === 'guests' && result.success && result.data"
        class="mt-2 text-sm text-green-700 space-y-1"
      >
      ...
      <!-- 일정 업로드 결과 -->
      <div
        v-if="type === 'schedules' && result.success && result.data"
        class="mt-2 text-sm text-purple-700 space-y-1"
      >
        <p>✓ 생성된 템플릿: {{ result.data.templates }}개</p>
        <div
          v-if="result.data.actions && result.data.actions.length > 0"
          class="mt-2"
        >
          <p class="font-medium">생성된 일정:</p>
          <ul class="ml-4 mt-1 space-y-1">
            <li
              v-for="(action, idx) in result.data.actions"
              :key="idx"
              class="text-xs"
            >
              • {{ action.actionType }}: {{ action.title }}
              <span class="text-purple-600"
                >({{ formatDateTime(action.scheduleDateTime) }})</span
              >
            </li>
          </ul>
        </div>
      ...
      <!-- 속성 업로드 결과 -->
      <div
        v-if="type === 'attributes' && result.success && result.data"
      ...
      >
        ...
        <div
          ...
          "
          class="mt-4 p-3 bg-white rounded border border-green-200"
        >
          <p class="font-medium text-green-900 mb-2">📊 속성별 통계</p>
          <div class="space-y-3">
            <div
              v-for="(values, key) in result.data.statistics"
              :key="key"
              class="text-xs"
            >
      ...
      <ul
        ...
        class="mt-3 text-sm text-yellow-700 bg-yellow-50 rounded p-2"
      >
        <li
      ...
      <ul
        ...
        class="mt-3 text-sm text-red-700 bg-red-50 rounded p-2"
      >
        <li
