<template>
  <div class="relative rounded-2xl overflow-hidden shadow-xl">
    <!-- 블러 배경 -->
    <div
      class="absolute inset-0 bg-gradient-to-br from-red-600 via-red-700 to-red-900"
    ></div>
...
const timeProgress = computed(() => {
  const total =
    deadlineDate.value.getTime() -
    (deadlineDate.value.getTime() - 30 * 24 * 60 * 60 * 1000) // 30일 기준
  const elapsed =
    now.value - (deadlineDate.value.getTime() - 30 * 24 * 60 * 60 * 1000)
