<template>
  <SlideUpModal :is-open="isOpen" @close="$emit('close')">
    ...
    <template #body>
      <form id="edit-field-form" class="space-y-2" @submit.prevent="handleSave">
        ...
        />
        <CommonDatePicker v-else v-model:value="internalValue" class="w-full" />
      </form>
    </template>
    <template #footer>
      <div class="flex gap-3 w-full">
        ...
        <button
          ...
          class="flex-1 py-3 px-4 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700"
        >
          저장
        </button>
      </div>
