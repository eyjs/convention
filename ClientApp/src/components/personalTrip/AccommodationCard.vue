<template>
  <div class="bg-white rounded-2xl shadow-md p-5">
    ...
    <div class="flex items-center gap-4 mb-4">
      <div
        class="flex-shrink-0 w-16 h-16 bg-gray-200 rounded-lg overflow-hidden flex items-center justify-center"
      >
        <!-- Hotel Icon -->
        <svg
        ...
        >
          <path
            ...
            d="M2.25 21h19.5m-19.5 0A2.25 2.25 0 0 0 5.25 18H19.5a2.25 2.25 0 0 0 2.25 2.25m-19.5 0A2.25 2.25 0 0 1 2.25 18V9.568m19.5 0a2.25 2.25 0 0 1 2.25 2.25V21M12 10.5v4.5m0-4.5H8.25m3.75 0h3.75M9 18h6"
          />
        </svg>
      </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4 border-t pt-4">
      <div>
        ...
        <p class="font-semibold text-gray-800 mt-1">
          {{ formatDateTime(accommodation.checkInTime) }}
        </p>
      </div>
      <div>
        <p class="text-xs text-gray-500">체크아웃</p>
        <p class="font-semibold text-gray-800 mt-1">
          {{ formatDateTime(accommodation.checkOutTime) }}
        </p>
      </div>
    </div>
    ...
    <div v-if="showActions" class="flex gap-2">
      <button
        class="flex-1 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition-colors"
        @click="$emit('select', accommodation)"
      >
        숙소 상세
      </button>
      <button
        class="flex-1 py-2.5 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 transition-colors"
        @click="openNavigation"
      >
        길찾기
      </button>
      <button
        class="p-2.5 bg-red-100 text-red-600 rounded-lg font-semibold hover:bg-red-200 transition-colors"
        title="삭제"
