    ...
    <template #body>
      <div v-if="accommodation" class="space-y-4">
        <div class="flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            class="p-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors flex-shrink-0"
            title="주소 복사"
        ...
        <p v-if="accommodation.postalCode" class="text-gray-600">
          우편번호: {{ accommodation.postalCode }}
        </p>
        <p class="text-gray-800 font-medium">
          체크인: {{ formatDateTime(accommodation.checkInTime) }}
        </p>
        <p class="text-gray-800 font-medium">
          체크아웃: {{ formatDateTime(accommodation.checkOutTime) }}
        </p>
        <p v-if="accommodation.expenseAmount" class="text-gray-800 font-medium">
          비용: ₩{{ accommodation.expenseAmount.toLocaleString() }}
        </p>
        <!-- Added this line -->
        <p v-if="accommodation.bookingReference" class="text-gray-700">
          예약 번호: {{ accommodation.bookingReference }}
        </p>
        <p v-if="accommodation.contactNumber" class="text-gray-700">
          연락처: {{ accommodation.contactNumber }}
        </p>
        <p v-if="accommodation.notes" class="whitespace-pre-wrap text-gray-700">
          {{ accommodation.notes }}
        </p>
        ...
        <div class="h-48 w-full rounded-lg mt-4">
          <KakaoMap
            v-if="
              isDomestic && accommodation.latitude && accommodation.longitude
            "
            :latitude="accommodation.latitude"
            :longitude="accommodation.longitude"
          />
          <GoogleMapPlaceholder
            v-else-if="!isDomestic && accommodation.latitude"
          />
        </div>
      </div>
    </template>
