<template>
  <SlideUpModal :is-open="show" z-index-class="z-[60]" @close="emit('close')">
        ...
        >
          ...
          <div v-if="!roundTrip.departure">
            <button
              ...
              @click="openFlightSearch('departure')"
            >
              <svg
                class="w-5 h-5"
                fill="none"
          ...
          <div v-else class="space-y-3">
            <FlightInfoDisplay
              ...
              type="departure"
              @clear="roundTrip.departure = null"
            />
            <div>
            ...
            <div>
              ...
              <input
                v-model.number="roundTrip.departure.amount"
                type="number"
                inputmode="numeric"
                class="input"
        ...
        >
          ...
          <div v-if="!roundTrip.arrival">
            <button
              ...
              @click="openFlightSearch('arrival')"
            >
              <svg
                class="w-5 h-5"
                fill="none"
          ...
          <div v-else class="space-y-3">
            ...
            <div>
              <label class="label">금액 (원)</label>
              <input
                v-model.number="roundTrip.arrival.amount"
                type="number"
                inputmode="numeric"
                class="input"
                placeholder="예: 150000"
    ...
    <template #footer>
      <div class="flex gap-3 w-full">
        ...
        <button
          :disabled="!roundTrip.departure && !roundTrip.arrival"
          class="btn-primary"
          @click="saveFlights"
        >
          저장
  ...
  <FlightSearchModal
    ...
    :flight-type="currentSearchType"
    @close="showFlightSearchModal = false"
    @apply="handleFlightSelect"
  />
</template>
  ...
  (newVal) => {
    if (newVal) {
      ...
      if (props.existingFlights && props.existingFlights.length > 0) {
        ...
        const dep = props.existingFlights.find(
          (f) =>
            f.departureLocation?.includes('인천') ||
            f.departureAirportCode === 'ICN',
        )
        const arr = props.existingFlights.find(
          (f) =>
            f.arrivalLocation?.includes('인천') ||
            f.arrivalAirportCode === 'ICN',
        )
...
function handleFlightSelect(flightInfo) {
  ...
  if (currentSearchType.value === 'departure') {
    const departureDateTime = dayjs(
      `${formatDate(flightInfo.scheduleDate)} ${flightInfo.estimatedTime || flightInfo.scheduleTime}`,
    )
...
.btn-primary {
  @apply flex-1 py-3 px-4 bg-primary-500 text-white rounded-xl font-semibold hover:bg-primary-600 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed;
}
.btn-secondary {
  @apply flex-1 py-3 px-4 bg-gray-100 text-gray-700 rounded-xl font-semibold hover:bg-gray-200 active:bg-gray-300 transition-colors;
}
</style>
