        ...
        >
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              @click="onAddWithCategory(category.name)"
            >
              + {{ category.name }} 추가
            </button>
          </div>
          ...
          <div
            v-if="getFlightsByCategory(category.name).length > 0"
            class="space-y-2"
          >
            <div
                ...
                <div class="flex-1 min-w-0">
                  ...
                  >
                    <div class="text-xs text-gray-500 mt-1 space-y-0.5">
                      <p v-if="flight.rentalCost">
                        렌트: ₩{{ formatAmount(flight.rentalCost) }}
                      </p>
                      <p v-if="flight.fuelCost">
                        주유: ₩{{ formatAmount(flight.fuelCost) }}
                      </p>
                      <p v-if="flight.tollFee">
                        톨비: ₩{{ formatAmount(flight.tollFee) }}
                      </p>
                      <p v-if="flight.parkingFee">
                        주차: ₩{{ formatAmount(flight.parkingFee) }}
                      </p>
                    </div>
                  </template>

                  <p class="text-sm font-medium text-primary-600 mt-1.5">
                    ₩{{ formatAmount(flight.amount || 0) }}
                  </p>
                </div>
                <div class="flex-shrink-0 flex gap-2 ml-4">
                  <button
                    class="p-2 text-primary-600 hover:bg-primary-100 rounded-full transition-colors"
                    @click="onEdit(flight)"
                  >
