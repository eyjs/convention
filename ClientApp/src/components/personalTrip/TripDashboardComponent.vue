<template>
  <div class="space-y-6">
    ...
    <section class="grid grid-cols-2 gap-4">
      <!-- 일정 카드 -->
      <div
        class="bg-white rounded-xl shadow-md p-5 cursor-pointer hover:shadow-lg transition-shadow"
        @click="$emit('go-to-itinerary')"
      >
      ...
      <div
        class="bg-white rounded-xl shadow-md p-5 cursor-pointer hover:shadow-lg transition-shadow"
        @click="$emit('open-accommodation-modal')"
      >
        <div class="flex items-center justify-between mb-3">
          <div
    ...
    <!-- Checklist Progress -->
    <section
      v-if="totalChecklistItems > 0 && checklistProgressPercentage < 100"
      class="bg-white rounded-2xl shadow-md p-6"
    >
      <div class="cursor-pointer" @click="$emit('go-to-notes')">
    ...
    <section v-if="trip.budget > 0" class="bg-white rounded-2xl shadow-md p-6">
      ...
      <div class="mb-4">
        <div class="flex justify-between text-sm text-gray-600 mb-2">
          ...
          <span
            >지출: {{ formatCurrency(totalExpenses) }} ({{
              budgetPercentage
            }}%)</span
          >
        </div>
        ...
        >
          ⚠️ 예산을
          {{ formatCurrency(totalExpenses - trip.budget) }} 초과했습니다.
        </p>
...
const todayItineraries = computed(() => {
  ...
  const currentTripDate = dayjs(props.trip.startDate).add(
    ...
    .filter((item) => {
      ...
      if (item.endTime) {
        const itemEndTime = dayjs(
          `${currentTripDate.format('YYYY-MM-DD')}T${item.endTime}`,
        )
  ...
  return filtered.map((item) => {
    const itemStartTime = dayjs(
      `${currentTripDate.format('YYYY-MM-DD')}T${item.startTime}`,
    )
    const itemEndTime = dayjs(
      `${currentTripDate.format('YYYY-MM-DD')}T${item.endTime}`,
    )
...
const accommodationCount = computed(
  () => props.trip.accommodations?.length || 0,
)
...
const expensesByCategory = computed(() => {
  ...
  if (props.trip.accommodations) {
    const total = props.trip.accommodations.reduce(
      (sum, acc) => sum + (acc.expenseAmount || 0),
      0,
    )
...
const totalChecklistItems = computed(() => {
  return (
    props.trip.checklistCategories?.reduce(
      (sum, category) => sum + category.totalItemsCount,
      0,
    ) || 0
  )
})
const completedChecklistItems = computed(() => {
  return (
    props.trip.checklistCategories?.reduce(
      (sum, category) => sum + category.completedItemsCount,
      0,
    ) || 0
  )
})
const checklistProgressPercentage = computed(() => {
  ...
  return Math.round(
    (completedChecklistItems.value / totalChecklistItems.value) * 100,
  )
})
</script>
