<template>
  <SlideUpModal :is-open="show" z-index-class="z-[70]" @close="emit('close')">
    <template #header-title
      >항공편 검색 ({{
        flightType === 'departure' ? '출발' : '도착'
      }})</template
    >
    <template #body>
      <div class="space-y-4">
        ...
        <div class="flex gap-2">
          <button
            :disabled="
              isLoading || !searchForm.date || !searchForm.flightNumber
            "
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1">
                ...
                <div class="flex items-center gap-2 text-sm text-gray-600 mb-1">
                  <span v-if="flightResult.type === 'DEPARTURE'"
                    >인천국제공항</span
                  >
                  <span v-else
                    >{{ flightResult.airport }} ({{
                      flightResult.airportCode
                    }})</span
                  >
                  <svg
                  ...
                  <span v-if="flightResult.type === 'ARRIVAL'"
                    >인천국제공항</span
                  >
                  <span v-else
                    >{{ flightResult.airport }} ({{
                ...
                <div class="flex items-center gap-3 text-xs text-gray-500">
                  <span
                    >{{ formatDate(flightResult.scheduleDate) }}
                    {{
                      flightResult.estimatedTime || flightResult.scheduleTime
                    }}</span
                  >
                  <span v-if="flightResult.terminal"
                    >터미널: {{ flightResult.terminal }}</span
                  >
                  <span v-if="flightResult.gate"
                    >게이트: {{ flightResult.gate }}</span
                  >
                </div>
    ...
    <template #footer>
      <button
        class="w-full py-3 px-4 bg-gray-100 text-gray-700 rounded-xl font-semibold hover:bg-gray-200 transition-colors"
        @click="emit('close')"
      >
        닫기
      </button>
...
async function searchFlight() {
  ...
  try {
    ...
    if (error.response?.status === 404) {
      errorMessage.value =
        '해당 항공편을 찾을 수 없습니다. 항공편 번호와 날짜를 확인해주세요.'
