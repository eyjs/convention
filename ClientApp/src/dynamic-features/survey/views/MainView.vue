<template>
  <div class="survey-page max-w-4xl mx-auto">
    ...
    <div v-else class="bg-white rounded-lg shadow-md p-6">
      <button @click="selectedSurvey = null" class="mb-4 text-blue-600 hover:text-blue-800">
        ← 목록으로
      </button>

      <h2 class="text-2xl font-bold text-gray-900 mb-2">{{ selectedSurvey.title }}</h2>
      <p class="text-gray-600 mb-6">{{ selectedSurvey.description }}</p>

      <div class="space-y-6">
        <div v-for="(question, index) in selectedSurvey.questions" :key="question.id" class="border-b pb-6">
          ...
          <div v-if="question.type === 'multiple-choice'" class="space-y-2">
            <label v-for="option in question.options" :key="option.id" 
              ...
              <input type="radio" :name="`q${question.id}`" :value="option.id" 
                     v-model="userResponse[question.id]" class="w-4 h-4">
              <span>{{ option.text }}</span>
            </label>
          </div>

          <textarea v-else-if="question.type === 'text'" 
...
async function submitResponse() {
  const unanswered = selectedSurvey.value.questions.filter(q => !userResponse.value[q.id])
  ...
  try {
    await axios.post(`/api/surveys/${selectedSurvey.value.id}/responses`, userResponse.value)
