...
export const chatbotAdminAPI = {
  ...
  updateLlmProvider: (id, data) =>
    apiClient.put(`/admin/llm-providers/${id}`, data),
  deleteLlmProvider: (id) => apiClient.delete(`/admin/llm-providers/${id}`),
  swapLlmProvider: (id) =>
    apiClient.patch(`/admin/llm-providers/${id}/toggle`, { isActive: true }),
  ...
  // 색인 상세 정보
  getIndexedItems: (conventionId) =>
    apiClient.get(`/admin/chatbot/conventions/${conventionId}/indexed-items`),

  // 재색인
  reindexAll: () => apiClient.post('/admin/chatbot/reindex-all'),
  reindexConvention: (conventionId) =>
    apiClient.post(`/admin/chatbot/reindex-convention/${conventionId}`),

  // 챗봇 토글
  toggleChatbot: (conventionId, enabled) =>
    apiClient.put(`/admin/chatbot/convention/${conventionId}/toggle`, {
      enabled,
    }),
