          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div class="text-sm text-gray-600 space-y-1">
                ...
                <p v-if="action.deadline">
                  <strong>마감:</strong> {{ formatDate(action.deadline) }}
                </p>
                <p>
                  <strong>완료율:</strong> {{ action.completedCount }} /
                  {{ action.totalGuestCount }} ({{ completionRate(action) }}%)
                </p>
              </div>
            </div>
        ...
        >
          ...
          <form @submit.prevent="createCustomAction" class="p-6 space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500">
                  영문 대문자, 숫자, 언더스코어만 사용 (예: CUSTOM_BUTTON_001)
                </p>
              </div>
            ...
            <div v-if="customActionForm.actionCategory">
              ...
              <p
                v-if="customActionForm.targetLocation"
            ...
            <div>
              ...
              <p
                v-else-if="jsonValidationSuccess"
...
async function fetchData() {
  ...
  try {
    const response = await apiClient.get(
      `/admin/action-management/convention/${conventionId}`,
    )
...
async function applyTemplate(template) {
  try {
    await apiClient.post(
      `/admin/action-templates/${template.id}/apply-to-convention/${conventionId}`,
      {
        isActive: true,
        isRequired: false,
        orderNum: actions.value.length,
...
async function toggleActive(action) {
  try {
    const response = await apiClient.put(
      `/admin/action-management/actions/${action.id}/toggle`,
    )
