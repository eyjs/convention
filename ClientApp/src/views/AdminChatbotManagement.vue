      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center">
            <button @click="router.push('/admin')" class="p-2 hover:bg-gray-100 rounded-lg">
          ...
          <button @click="fetchAllData" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900 mt-1">{{ stats.activeConventions }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900 mt-1">{{ stats.totalGuests }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900 mt-1">{{ formatBytes(stats.dbSize) }}</p>
            </div>
              ...
              >
                <div class="flex items-start justify-between">
                  ...
                  <div class="flex gap-2">
                    ...
                    <button
                      @click="toggleChatbot(convention.id, !convention.chatbotEnabled)"
                      :class="[
...
async function fetchStats() {
  try {
    const response = await apiClient.get('/admin/chatbot/stats')
...
async function fetchConventions() {
  try {
    const response = await apiClient.get('/admin/chatbot/conventions')
...
async function fetchVectorStats() {
  try {
    const response = await apiClient.get('/admin/chatbot/vector-stats')
...
async function fetchRecentActivities() {
  try {
    const response = await apiClient.get('/admin/chatbot/recent-activities')
...
async function fetchLogs() {
  try {
    const response = await apiClient.get('/admin/chatbot/logs')
...
async function handleFullReindex() {
  ...
  try {
    const response = await apiClient.post('/admin/chatbot/reindex-all')
    reindexResult.value = {
      success: true,
...
async function reindexConvention(conventionId) {
  try {
    await apiClient.post(`/admin/chatbot/reindex-convention/${conventionId}`)
...
async function toggleChatbot(conventionId, enabled) {
  try {
    await apiClient.put(`/admin/chatbot/convention/${conventionId}/toggle`, { enabled })
    await fetchConventions()
  } catch (error) {
