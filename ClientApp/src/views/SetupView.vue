<template>
  <div class="min-h-screen min-h-dvh bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
    <div class="max-w-md w-full">
      <div class="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div v-if="!created" class="space-y-6">
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div class="flex items-start">
              ...
              <div class="ml-3">
                <h3 class="text-sm font-medium text-blue-800">생성될 계정 정보</h3>
        ...
        <div v-else class="space-y-6">
          <div class="bg-green-50 border border-green-200 rounded-lg p-6">
            ...
            <h3 class="text-center text-lg font-semibold text-green-900 mb-2">계정 생성 완료!</h3>
            <div class="bg-white rounded-lg p-4 mb-4">
              <p class="text-sm text-gray-600 mb-2">로그인 정보:</p>
              <div class="space-y-1 font-mono text-sm">
                <p><span class="text-gray-500">아이디:</span> <strong>{{ adminInfo.loginId }}</strong></p>
                <p><span class="text-gray-500">비밀번호:</span> <strong>{{ adminInfo.password }}</strong></p>
              </div>
            </div>
            <p class="text-xs text-center text-gray-600">이 정보를 안전한 곳에 보관하세요</p>
          </div>
      ...
      <p class="text-center text-sm text-gray-600 mt-6">
        이미 계정이 있으신가요? 
        <router-link to="/login" class="text-indigo-600 hover:text-indigo-700 font-medium">로그인</router-link>
      </p>
    </div>
  </div>
</template>
...
const createAdmin = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || '계정 생성 중 오류가 발생했습니다.';
  } finally {
    loading.value = false;
  }
};
