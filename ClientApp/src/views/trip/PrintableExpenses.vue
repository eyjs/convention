<template>
  <div class="p-10 bg-white" style="width: 210mm">
    <!-- Header -->
    <header
      class="flex items-center justify-between pb-4 border-b-2 border-gray-800"
    >
      <div>
    ...
    <main class="mt-8">
      ...
      <section class="mb-10">
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div
            ...
            :class="{
              'bg-green-50': trip.budget - totalExpenses >= 0,
              'bg-orange-50': trip.budget - totalExpenses < 0,
            }"
          >
            <p
              class="text-sm font-semibold"
              :class="{
                'text-green-800': trip.budget - totalExpenses >= 0,
                'text-orange-800': trip.budget - totalExpenses < 0,
              }"
            >
              남은 예산
            </p>
            <p
              ...
              :class="{
                'text-green-900': trip.budget - totalExpenses >= 0,
                'text-orange-900': trip.budget - totalExpenses < 0,
              }"
            >
      ...
      <section class="mb-10">
        ...
        >
          <h4
            class="text-lg font-semibold bg-gray-100 p-3 rounded-t-lg flex justify-between"
          >
            <span
              >Day {{ day.dayNumber }} ({{
      ...
      >
        <h3
          class="text-xl font-semibold bg-gray-100 p-3 rounded-t-lg flex justify-between mb-4"
        >
...
const totalExpenses = computed(() => {
  ...
  if (props.trip.itineraryItems)
    total += props.trip.itineraryItems.reduce(
      (sum, item) => sum + (item.expenseAmount || 0),
      0,
    )
  if (props.trip.accommodations)
    total += props.trip.accommodations.reduce(
      (sum, acc) => sum + (acc.expenseAmount || 0),
      0,
    )
  if (props.trip.flights)
    total += props.trip.flights.reduce((sum, f) => sum + getFlightAmount(f), 0)
  return total
})

const categoryStats = computed(() => {
  const itineraryItems =
    props.trip.itineraryItems?.filter((i) => i.expenseAmount > 0) || []
  const accommodationItems =
    props.trip.accommodations?.filter((a) => a.expenseAmount > 0) || []
  const transportationItems =
    props.trip.flights?.filter((f) => getFlightAmount(f) > 0) || []
  return [
    {
      name: '일정',
      key: 'itinerary',
...
const dailyExpenses = computed(() => {
  ...
  props.trip.flights
    ...
    .forEach((taxi) => {
      const itinerary = props.trip.itineraryItems?.find(
      ...
      if (itinerary && itinerary.dayNumber) {
        ...
        if (day) {
          ...
          day.items.push({
            id: `fl-${taxi.id}`,
            type: '교통(택시)',
            description: `${taxi.departureLocation || ''} → ${taxi.arrivalLocation || ''}`,
            amount,
          })
          day.total += amount
        }
...
const tripWideExpenses = computed(() => {
  ...
  const accommodationTotal =
    props.trip.accommodations?.reduce(
      (sum, acc) => sum + (acc.expenseAmount || 0),
      0,
    ) || 0
