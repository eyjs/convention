        ...
        <section class="bg-white rounded-2xl shadow-md p-5">
          ...
          <div v-if="groupedItinerary.length > 0" class="relative">
            ...
            >
              <button
                :class="[
                  'flex items-center justify-center px-5 py-2.5 rounded-xl font-semibold text-sm whitespace-nowrap transition-all',
                  selectedDay === null
                    ? 'text-white shadow-md'
                    : 'bg-white shadow-md text-gray-600 hover:shadow-lg hover:text-gray-800',
                ]"
                :style="
                  selectedDay === null
                    ? 'background-color: rgba(23, 177, 133, 1);'
                    : ''
                "
          ...
          <div v-if="groupedItinerary.length === 0" class="text-center py-12">
            ...
            <button
              ...
              class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-500 text-white rounded-full font-semibold hover:shadow-lg transition-shadow"
          ...
          <div v-else class="space-y-6">
            <div
              v-for="dayGroup in filteredItinerary"
            ...
            >
              ...
              <div class="mb-4">
                <div class="flex items-center justify-between mb-2">
                  ...
                  >
                    ...
                    <button
                      ...
                      :class="
                        isEditModeForDay(dayGroup.dayNumber)
                          ? 'text-primary-600 hover:text-primary-700'
                          : 'text-gray-600 hover:text-gray-700'
                      "
                      @click="toggleEditMode(dayGroup.dayNumber)"
                    >
                      {{
                ...
                <div class="flex items-center gap-3">
                  <span class="text-sm text-gray-500">
                    {{
                      dayGroup.items.filter((i) => i.type === 'itinerary')
                        .length
                    }}개 일정
                    <span
                      v-if="
              ...
              <div>
                <div
                  v-for="(item, index) in dayGroup.items"
                  :key="item.id"
                  class="flex gap-3"
                >
                  ...
                  >
                    ...
                    >
                      <Check
                        v-if="selectedItineraryItems.includes(item.id)"
                  ...
                  <div
                    v-if="!isEditModeForDay(dayGroup.dayNumber)"
                    class="relative flex-shrink-0 w-5 flex flex-col items-center"
                  >
                    ...
                    <div
                      ...
                      style="
                        width: 0px;
                        border-right: 1px dashed rgba(23, 177, 133, 0.5);
                      "
                    ></div>
                    ...
                    >
                      ...
                      <div
                        ...
                        style="border-right: 1px dashed rgba(23, 177, 133, 0.5)"
                      ></div>
                      ...
                      <div
                        ...
                        style="
                          background-color: rgba(23, 177, 133, 0.1);
                          color: rgba(23, 177, 133, 1);
                        "
                      >
                  ...
                  <div class="flex-1 pb-6">
                    <div
                      :ref="
                        (el) => {
                          if (item.id === currentItineraryItemId)
                            currentItineraryItemRef = el
                        }
                      "
                      ...
                      :style="
                        currentItineraryItemId === item.id
                          ? 'border: 2px solid rgba(23, 177, 133, 1); box-shadow: 0 4px 6px -1px rgba(23, 177, 133, 0.1);'
                          : ''
                      "
                      @click="
                        item.type === 'transportation'
                          ? router.push(`/trips/${tripId}/transportation`)
                          : !isEditModeForDay(dayGroup.dayNumber) &&
                            openItineraryDetailModal(item)
                      "
                    >
                      <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <div
                            v-if="
                              item.category || item.type === 'transportation'
                            "
                            class="mb-2 flex items-center gap-2"
                          >
                            <span
                              v-if="item.type === 'transportation'"
                              class="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full"
                            >
                              <Plane
                                v-if="item.category === '항공편'"
                                class="w-3 h-3"
                              />
                              <Train
                                v-else-if="item.category === '기차'"
                                class="w-3 h-3"
                              />
                              <Bus
                                v-else-if="item.category === '버스'"
                                class="w-3 h-3"
                              />
                            ...
                            <span
                              v-else
                              class="inline-flex items-center gap-1 px-2 py-1 bg-primary-50 text-primary-700 text-xs font-medium rounded-full"
                            >
                              <component
                          ...
                          <!-- Address (일반 일정만) -->
                          <p
                            v-if="item.address && item.type === 'itinerary'"
                          ...
                          <div
                            v-if="
                              item.type === 'transportation' && item.startTime
                            "
                            class="flex items-center gap-3 text-sm text-gray-600 mb-1"
                          >
                          ...
                          <div
                            v-else-if="
                              ...
                              item.endTime
                            "
                            class="flex items-center gap-1 text-sm text-gray-600 mb-1"
                          >
                            <svg
                              class="w-4 h-4 text-gray-400"
                              fill="none"
                        ...
                        <!-- Icons (Right-most) -->
                        <div
                          class="flex-shrink-0 ml-auto flex flex-col items-end gap-1"
                        >
                          ...
                          <div
                            ...
                            @dragstart.stop="
                              onDragStart(item, dayGroup.dayNumber, $event)
                            "
                            @drag.stop="onDrag"
                            ...
                            @touchstart.stop="
                              onTouchStart(item, dayGroup.dayNumber, $event)
                            "
                          >
                            <svg
                              class="w-6 h-6"
              ...
              >
                ...
                <button
                  ...
                  :class="
                    isAllSelectedForDay(dayGroup.dayNumber)
                ...
                >
                  선택 항목 삭제 ({{
                    getSelectedItemsForDay(dayGroup.dayNumber).length
                  }}개)
                </button>
              </div>
              ...
              <button
                v-if="
                  !effectiveReadonly && !isEditModeForDay(dayGroup.dayNumber)
                "
                class="w-full py-3 border-2 border-dashed rounded-xl font-medium hover:opacity-80 transition-all"
                style="
                  border-color: rgba(23, 177, 133, 0.3);
                  color: rgba(23, 177, 133, 1);
                  background-color: rgba(23, 177, 133, 0.05);
                "
                @click="openItineraryModal(null, dayGroup.dayNumber)"
              >
                + 일정 추가
              </button>
            </div>
        ...
        <template #body>
          <div v-if="selectedItinerary" class="space-y-4">
            ...
            <h3 class="text-xl font-bold">
              {{ selectedItinerary.locationName }}
            </h3>

            <!-- 주소 (길찾기 버튼) -->
            <div
              v-if="selectedItinerary.address"
            ...
            >
              ...
              <span
                >{{ selectedItinerary.expenseAmount.toLocaleString() }}원</span
              >
            </div>
...
const handleDayFilterScroll = () => {
  if (dayFilterContainer.value) {
    ...
    showRightDayScroll.value =
      ...
      dayFilterContainer.value.scrollWidth -
        dayFilterContainer.value.clientWidth
  }
}
  ...
  if (trip.value.flights) {
    trip.value.flights.forEach((flight) => {
      if (flight.departureTime) {
        ...
        const dayNumber =
          Math.floor((flightDateOnly - startDateOnly) / (1000 * 60 * 60 * 24)) +
          1
  ...
  try {
    ...
    if (isNewItem) {
      const response = await apiClient.post(
        `/personal-trips/${tripId.value}/items`,
        payload,
      )
...
async function saveItineraryOrder(dayNumber) {
  ...
  try {
    await apiClient.put(`/personal-trips/${tripId.value}/items/reorder`, {
      items: dayItems,
    })
...
function toggleEditMode(dayNumber) {
  ...
  if (!editModeByDay.value[dayNumber]) {
    const dayItems =
      trip.value.itineraryItems?.filter(
...
function getSelectedItemsForDay(dayNumber) {
  const dayItems =
    trip.value.itineraryItems?.filter((item) => item.dayNumber === dayNumber) ||
    []
