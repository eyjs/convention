<template>
  <div class="min-h-screen relative bg-gray-50">
    ...
    <div class="fixed inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      <div
        class="absolute -top-32 -left-32 w-80 h-80 bg-gradient-to-br from-sky-200/15 to-blue-200/15 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute top-1/2 -right-40 w-96 h-96 bg-gradient-to-br from-blue-200/12 to-cyan-200/12 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute bottom-32 left-1/4 w-72 h-72 bg-gradient-to-br from-cyan-200/12 to-sky-200/12 rounded-full blur-3xl"
      ></div>
    ...
    <div class="relative z-10">
      ...
      <div class="max-w-2xl mx-auto px-4 py-4">
        ...
        <template v-else>
          ...
          <div v-if="trips.length === 0" class="text-center py-16">
            <div
              class="w-24 h-24 mx-auto mb-6 bg-primary-100 rounded-full flex items-center justify-center"
            >
              <svg
            ...
            <button
              class="inline-flex items-center gap-2 px-6 py-3 bg-primary-500 text-white rounded-full font-semibold hover:shadow-lg transition-shadow"
          ...
          <div v-else class="space-y-5">
            ...
            >
              ...
              <div class="relative h-40 overflow-hidden">
                ...
                <div
                  v-else
                  class="absolute inset-0 bg-gradient-to-br from-cyan-500 via-teal-500 to-blue-600 pointer-events-none"
                >
                  <div
              ...
              <div class="p-5">
                ...
                <div class="flex gap-2 flex-wrap">
                  <span
                    v-if="trip.flights.length > 0"
                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs font-semibold"
                  >
                    <svg
                      class="w-3.5 h-3.5"
                      fill="none"
                  ...
                  <span
                    v-if="trip.accommodations.length > 0"
                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-xs font-semibold"
                  >
                    <svg
                      class="w-3.5 h-3.5"
                      fill="none"
      ...
      <!-- 새 여행 만들기 FAB -->
      <button
        class="fixed bottom-8 right-8 bg-primary-500 text-white rounded-full p-5 shadow-2xl hover:shadow-3xl focus:outline-none focus:ring-4 focus:ring-primary-300/50 transition-shadow z-20"
        @click="goToCreateTrip"
      >
...
async function loadTrips() {
  ...
  try {
    const response = await apiClient.get(
      `/personal-trips?_=${new Date().getTime()}`,
    )
