    ...
    <div class="max-w-2xl mx-auto px-4 py-4 pb-28">
      ...
      <div v-if="activeTab === 'checklist'">
        ...
        <div v-else class="space-y-4">
          <div
            v-for="(category, catIndex) in checklist"
            :key="category.id"
          ...
          >
            ...
            >
              ...
              <div class="flex items-center gap-2">
                <button
                  v-if="isEditMode && !category.isDefault"
            ...
            <div v-if="expandedCategories.includes(category.id)">
              ...
              <div class="px-4 pb-4 space-y-3">
                ...
                <div
                  ...
                  @click="!isEditMode && toggleItem(catIndex, itemIndex)"
                >
                  <div
                  ...
                  <div
                    class="flex-1"
                    @click.stop="isEditMode && openItemEditModal(item)"
                  >
                    <p class="text-gray-800">{{ item.task }}</p>
                    <p v-if="item.description" class="text-xs text-gray-500">
                      {{ item.description }}
                    </p>
                  </div>
                  <button
                    v-if="isEditMode"
                    class="p-1 text-red-500 hover:bg-red-100 rounded-full transition-opacity"
                    @click.stop="deleteItem(category.id, item.id)"
                  >
                    <svg
                      class="w-5 h-5"
                      fill="none"
                ...
                <div
                  ...
                  @click.stop="promptNewItem(category.id)"
                >
                  <div
                    class="flex-shrink-0 w-5 h-5 rounded-full border-2 border-dashed border-primary-500 group-hover:border-primary-400"
                  ></div>
          ...
          <div v-if="!effectiveReadonly" class="flex justify-center mt-6">
            <button
              class="w-1/2 py-3 bg-[#17B185] text-white rounded-xl font-semibold shadow-lg hover:bg-green-600 transition-all"
    ...
    <SlideUpModal :is-open="isInputModalOpen" @close="closeInputModal">
      ...
      <template #footer>
        <div class="flex gap-3 w-full">
          ...
          <button
            type="button"
            class="flex-1 py-3 px-4 bg-primary-500 text-white rounded-xl font-semibold"
    ...
    <SlideUpModal :is-open="isItemEditModalOpen" @close="closeItemEditModal">
      ...
      <template #body>
        <form
          ...
          id="item-edit-form"
          class="space-y-4"
          @submit.prevent="saveItemChanges"
        >
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >항목</label
...
async function saveItemChanges() {
  ...
  try {
    const response = await apiClient.put(
      `/personal-trips/checklist-items/${itemToSave.id}`,
      {
    ...
    if (name && name.trim()) {
      try {
        const response = await apiClient.post(
          `/personal-trips/${tripId.value}/checklist-categories`,
          { name: name.trim() },
        )
