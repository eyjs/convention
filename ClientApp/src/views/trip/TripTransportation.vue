  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="flex flex-wrap gap-2 mb-4">
        ...
        >
          ...
          <span class="text-xs opacity-80">{{
            getFlightsByCategory(category.name).length
          }}</span>
        </button>
      </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="text-left">
                ...
                <p class="text-xs text-gray-500">
                  {{ getFlightsByCategory(category.name).length }}건
                </p>
              </div>
            </div>
            <svg
              ...
              <div v-if="getFlightsByCategory(category.name).length > 0">
                <template v-if="category.name === '항공편'">
                  ...
                  >
                    ...
                    <div
                      class="h-1.5 w-full relative z-10"
                      :class="getStatusConfig(flight).bg"
                    ></div>
                    <div class="p-5 relative z-10">
                      <div class="flex justify-between items-start mb-6">
                        <div class="flex items-center gap-3">
                          <div
                            class="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-xs font-bold text-gray-700 shadow-sm border border-gray-100"
                          >
                            {{ getAirlineLogoText(flight) }}
                          </div>
                          <div>
                            <h3
                              class="font-bold text-gray-900 text-sm tracking-tight"
                            >
                              {{ flight.airline }}
                            </h3>
                            <div
                        ...
                        <div
                          class="px-2.5 py-1 rounded-full text-[11px] font-bold tracking-tight flex items-center gap-1.5 shadow-sm"
                          :class="[
                            getStatusConfig(flight).badgeBg,
                            getStatusConfig(flight).text,
                          ]"
                        >
                          <component
                            :is="getStatusConfig(flight).icon"
                            class="w-3 h-3"
                          />
                      ...
                      <div class="flex items-center justify-between mb-6 px-1">
                        <div class="text-left">
                          <div
                            class="text-3xl font-black text-gray-800 tracking-tight"
                          >
                            {{
                        ...
                        <div class="flex-1 px-4 flex flex-col items-center">
                          <div
                            class="text-[10px] text-gray-400 font-medium mb-1"
                          >
                            직항
                          </div>
                          <div
                            class="w-full h-px bg-gray-200 relative flex items-center justify-center"
                          >
                            <Plane
                              class="w-5 h-5 text-indigo-400 rotate-90 absolute z-0"
                            />
                            <div
                              class="w-1.5 h-1.5 rounded-full bg-gray-300 absolute left-0"
                            ></div>
                            <div
                              class="w-1.5 h-1.5 rounded-full bg-gray-300 absolute right-0"
                            ></div>
                          </div>
                          <div
                        ...
                        <div class="text-right">
                          ...
                          <div class="flex flex-col items-end mt-1">
                            ...
                            <span
                              v-if="isDelayed(flight)"
                              class="text-[10px] text-gray-400 line-through"
                              >{{ formatTime(flight.arrivalTime) }}</span
                            >
                          </div>
                        </div>
                      </div>
                      ...
                      <div
                        class="bg-gray-50/80 backdrop-blur-[2px] rounded-2xl p-4 grid grid-cols-2 gap-y-4 gap-x-2 border border-gray-100/50"
                      >
                        <div class="flex flex-col">
                          <span
                            class="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"
                            ><Calendar class="w-3 h-3" /> 날짜</span
                          >
                          <span class="text-sm font-bold text-gray-700">{{
                            formatDate(
                              getFlightType(flight) === 'Arrival'
                                ? flight.arrivalTime
                        ...
                        <div
                          class="flex flex-col border-l border-gray-200 pl-4"
                        >
                          <span
                            class="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"
                            ><DoorOpen class="w-3 h-3" /> 탑승구</span
                          >
                        ...
                        <div class="flex flex-col">
                          <span
                            class="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"
                            ><Building2 class="w-3 h-3" /> 터미널</span
                          >
                          <span class="text-sm font-bold text-gray-700">{{
                            flight.terminal || 'N/A'
                          }}</span>
                        ...
                        >
                          <span
                            class="text-[10px] text-gray-400 font-bold mb-1 flex items-center gap-1"
                            ><MapPin class="w-3 h-3" /> 예약번호</span
                          >
                          <span class="text-sm font-bold text-gray-700">{{
                            flight.bookingReference || '없음'
                          }}</span>
                        </div>
                      </div>
                      ...
                      <div class="mt-4 flex items-center justify-center">
                        <div
                          class="text-xs text-gray-500 font-medium bg-white/90 px-3 py-1 rounded-full border border-gray-100 shadow-sm flex items-center gap-1 backdrop-blur-sm"
                        >
                ...
                <template v-else>
                  ...
                  <template v-if="category.name === '기차'">
                    <div
                      ...
                      :class="{
                        'cursor-pointer hover:shadow-xl': !effectiveReadonly,
                      }"
                      @click="!effectiveReadonly && openDetailModal(flight)"
                    >
                      <div class="p-6">
                        ...
                        <!-- Route (모바일 최적화) -->
                        <div
                          class="bg-white rounded-xl p-3 md:p-5 mb-4 shadow-sm"
                        >
                          <div
                            class="flex items-center justify-between gap-2 md:gap-4"
                          >
                            <div class="flex-1 min-w-0">
                              ...
                              <div
                                class="text-sm md:text-lg font-black text-gray-900 truncate"
                              >
                                {{ flight.departureLocation || '출발역' }}
                              </div>
                              <div
                                class="text-xs md:text-sm font-bold text-green-600 mt-0.5"
                              >
                                {{ formatTime(flight.departureTime) }}
                              </div>
                            </div>
                            ...
                            <div
                              class="flex flex-col items-center px-1 flex-shrink-0"
                            >
                              <Train
                                class="w-4 h-4 md:w-6 md:h-6 text-green-500 mb-0.5 md:mb-1"
                              />
                        ...
                        <div
                          v-if="flight.notes"
                          class="mt-4 p-3 bg-white rounded-lg border border-green-200"
                        >
                    ...
                    <div class="flex justify-between items-start">
                      <div class="flex-1 min-w-0">
                        ...
                        <template v-else-if="flight.category === '택시'">
                          ...
                          <div
                            v-if="getLinkedItinerary(flight.itineraryItemId)"
                            class="text-xs text-gray-500"
                          >
                            연결된 일정:
                            {{
                              getLinkedItinerary(flight.itineraryItemId)
                                ?.locationName
                            }}
                          </div>
                        </template>
                        ...
                        <template
                          v-else-if="
                            ['렌트카', '자가용'].includes(flight.category)
                          "
                        >
                          <div
                            v-if="flight.airline"
                          ...
                          >
                            {{ formatDate(flight.departureTime) }}
                            <span v-if="flight.arrivalTime">
                              ~ {{ formatDate(flight.arrivalTime) }}</span
                            >
                          </div>
                        </template>
    ...
    >
      <template #header-title>{{
        editingFlight?.id ? '교통편 수정' : '교통편 추가'
      }}</template>
      <template #body>
        <form
          id="transportation-form"
          class="space-y-4"
          @submit.prevent="saveTransportation"
        >
          <!-- 카테고리 (읽기전용) -->
          <div>
            <label class="label">카테고리</label>
            <input
              v-model="flightData.category"
              type="text"
          ...
          <template v-if="flightData.category === '항공편'">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  type="text"
                  class="input"
                  :class="{ 'bg-gray-100': isApiSourcedFlight }"
                  placeholder="대한항공, 아시아나 등"
                  :readonly="isApiSourcedFlight"
                />
              </div>
              <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  v-model="flightData.departureLocation"
                  type="text"
                  class="input"
                  :class="{ 'bg-gray-100': isApiSourcedFlight }"
                  placeholder="출발 공항 코드"
                  :readonly="isApiSourcedFlight"
                />
              </div>
              <div>
                ...
                <input
                  v-model="flightData.arrivalLocation"
                  type="text"
                  class="input"
                  :class="{ 'bg-gray-100': isApiSourcedFlight }"
                  placeholder="도착 공항 코드"
                  :readonly="isApiSourcedFlight"
                />
              </div>
            </div>
            <div>
              ...
              <CommonDatePicker
                ...
                :disabled="
                  isApiSourcedFlight &&
                  getFlightType(editingFlight) === 'Departure'
                "
                :class="{
                  'bg-gray-100':
                    isApiSourcedFlight &&
            ...
            <div>
              ...
              <CommonDatePicker
                ...
                :disabled="
                  isApiSourcedFlight &&
                  getFlightType(editingFlight) === 'Arrival'
                "
                :class="{
                  'bg-gray-100':
                    isApiSourcedFlight &&
            ...
            <div>
              ...
              <input
                v-model="flightData.bookingReference"
                type="text"
                class="input"
                placeholder="예약번호 (선택)"
              />
          ...
          <template v-else-if="flightData.category === '버스'">
            ...
            <div>
              ...
              <input
                v-model="flightData.departureDate"
                type="date"
                class="input"
              />
            </div>
          ...
          <template v-else-if="flightData.category === '택시'">
            <div>
              ...
              <select v-model="flightData.itineraryItemId" class="input">
                ...
                <option
                  v-for="item in trip.itineraryItems"
                  :key="item.id"
                  :value="item.id"
                >
      ...
      <template #footer>
        <div class="flex gap-3 w-full">
          <button
            v-if="editingFlight?.id"
            type="button"
            class="py-3 px-4 bg-red-100 text-red-700 rounded-xl font-semibold hover:bg-red-200 transition-colors"
          ...
          <button
            ...
            class="flex-1 py-3 px-4 bg-primary-500 text-white rounded-xl font-semibold hover:bg-primary-600 transition-all"
          >
            저장
          </button>
        </div>
...
const getFlightType = (flight) => {
  if (
    flight.arrivalLocation?.toUpperCase().includes('ICN') ||
    flight.arrivalLocation?.includes('인천')
  ) {
    return 'Arrival'
  }
  return 'Departure'
...
const getAirlineLogoText = (flight) =>
  flight.airline ? flight.airline.substring(0, 2).toUpperCase() : ''
const isDelayed = (flight) => flight.status?.toLowerCase() === 'delayed'
const getArrivalCity = (flight) =>
  flight.arrivalLocation ? flight.arrivalLocation.split('(')[0].trim() : ''

const formatTime = (time) => (time ? dayjs(time).format('HH:mm') : '--:--')
const formatDate = (date) =>
  date ? dayjs(date).format('YYYY-MM-DD') : '날짜 정보 없음'
...
const categoriesWithData = computed(() => {
  return categories.filter(
    (category) => getFlightsByCategory(category.name).length > 0,
  )
})
...
function getCategoryTotal(category) {
  return getFlightsByCategory(category).reduce(
    (sum, f) => sum + getFlightTotalCost(f),
    0,
  )
}
...
function openDetailModal(flight) {
  ...
  flightData.value = {
    ...
    departureTime: flight.departureTime
      ? dayjs(flight.departureTime).format('YYYY-MM-DDTHH:mm:ss')
      : '',
    arrivalTime: flight.arrivalTime
      ? dayjs(flight.arrivalTime).format('YYYY-MM-DDTHH:mm:ss')
      : '',
    departureDate: flight.departureTime
      ? dayjs(flight.departureTime).format('YYYY-MM-DD')
      : '',
  }
  isEditModalOpen.value = true
}
...
async function saveTransportation() {
  try {
    ...
    if (
      flightData.value.category === '버스' &&
      flightData.value.departureDate
    ) {
    ...
    if (editingFlight.value?.id) {
      await apiClient.put(
        `/personal-trips/flights/${editingFlight.value.id}`,
        payload,
      )
    } else {
